...
func main() {
	...
	// Pause controller container
	if err := cli.ContainerPause(ctx, cont.ID); err != nil {
	...
	for {
		...
		if err := cli.ContainerUnpause(ctx, cont.ID); err != nil {
			log.Fatalf("Unable to unpause controller container: %s\n", err.Error())
		}
